...
The current machine is completely offline. It has no Internet access and cannot download packages, repositories, documentation, binaries, models, or other external resources.

Do not attempt any network operation, including but not limited to:

- apt, apt-get, or other system package managers;

- pip install, npm install, cargo install, or similar language package managers;

- git clone, git fetch, or access to remote repositories;

- curl, wget, or requests to external URLs;

- downloading compilers, debuggers, fuzzers, libraries, dictionaries, corpora, or prebuilt tools.

Do not spend time retrying failed network operations. Do not suggest that the user install missing dependencies. Complete the task using only the files and commands already available inside the environment.
"""

CLAUDE_LOG_DIR = "/home/claude/cc_log"
...
def run_with_configs(claude_code_args: ClaudeCodeArgs, task_args: TaskArgs):
    ...
    eval_id = list(filter(lambda x: "glm" in x.lower() or "jt236" in x.lower(), str(claude_code_args.log_dir).split("/")))[0]
