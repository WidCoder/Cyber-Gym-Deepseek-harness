...
class OpenAIChatAggregator:
    """Aggregate OpenAI-compatible chat-completion SSE chunks."""
    ...
    def __init__(self) -> None:
        ...
        self.started = False
        self.errors: list[str] = []
        self.provider_response_id: Optional[str] = None
    ...
    @property
    def message_id(self) -> Optional[str]:
        # OpenAI completion IDs are provider IDs, not Harness session IDs.
        return None
    ...
    def consume(self, payload: Any) -> None:
        ...
        for key in ("content", "reasoning_content", "reasoning"):
            ...
            if isinstance(value, str):
                self.message[key] = str(self.message.get(key) or "") + value
        ...
        if isinstance(tool_calls, list):
            for call in tool_calls:
                ...
                target = self._tool_calls.setdefault(
                    index,
                    {
                        "id": None,
                        "type": "function",
    ...
    def snapshot(self) -> Optional[dict[str, Any]]:
        ...
        if self._tool_calls:
            result["tool_calls"] = [
                copy.deepcopy(self._tool_calls[index])
...
class SSEDecoder:
    ...
    def _dispatch(self) -> Optional[dict[str, Any]]:
        ...
        if data_text.strip() == "[DONE]":
            # OpenAI-compatible SSE uses [DONE] instead of Anthropic
            # message_stop as the normal stream terminator.
            self.done = True
            mark_complete = getattr(self.aggregator, "mark_complete", None)
...
class RequestCapture:
    ...
    def start_response(
        ...
        *,
        ...
        if is_sse:
            path = str(self.request_record.get("path") or "").rstrip("/")
            if path in {"/v1/messages", "/messages"}:
                aggregator: Any = AnthropicMessageAggregator()
            else:
                aggregator = OpenAIChatAggregator()
            self._decoder = SSEDecoder(aggregator)
    ...
    def finalize(
        ...
        *,
        ...
        if aggregator is not None:
            ...
            provider_response_id = getattr(aggregator, "provider_response_id", None)
